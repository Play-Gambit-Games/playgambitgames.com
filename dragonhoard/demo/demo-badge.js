 ...
 *
 * The development-status wording (pill and first bullet) lives HERE rather than in the site that
 * hosts the demo, because build-demo.mjs copies this file into a fresh copy of the game build on
 * every run. Anything the host writes into the demo's body is destroyed by the next rebuild; this
 * is regenerated by it. Take it out only when the game actually ships.
 *
...
(function () {
	...
	style.textContent = [
		...
		'  pointer-events: auto;',
		'}',
		/* The panel is a column: prose scrolls, the buttons do not. Before the early-build note the
		   content happened to fit a desktop window without scrolling, so the dismiss button was
		   always in view; now it does not fit, and on a phone it never did. Keeping the button row
		   out of the scrolling region is the fix that holds at every size. */
		'.panel {',
		'  width: min(520px, 100%);',
		...
		'}',
		// min-height:0 is what actually lets this shrink below its content inside the flex column.
		'.body { overflow-y: auto; -webkit-overflow-scrolling: touch; min-height: 0; }',
		'.panel h2 {',
		...
		'.facts { margin: 0; border-top: 1px solid rgba(255,255,255,0.09); }',
		...
		'.actions { display: flex; gap: 8px; flex-wrap: wrap; flex: none; padding-top: 14px; }',
		'button {',
	...
	// "Early build" rides in the always-visible part of the pill, not in the .hint, because the
	// hint is what narrow phones drop. A player on a 360px screen still reads all three facts:
	// it is a demo, the money is not real, and the game is not finished.
	pill.innerHTML =
		'<span class="dot"></span><span>Demo &middot; Play money &middot; Early build</span>' +
	...
	function openPanel() {
		...
		panel.innerHTML =
			...
			', a game still in development. It is not a casino, and nothing here can be won or ' +
			'lost.</p>' +
			'<ul>' +
			'<li><strong>This is an early build.</strong> ' +
			(CONFIG.gameTitle || 'The game') +
			' is still being made: art, sound, balancing and the interface are all still changing, ' +
			'so expect rough edges and details that will not match the finished game. It has not ' +
			'been released, and it cannot be played for real money anywhere.</li>' +
		...
		// preventScroll is belt and braces now that the button row sits outside the scrolling body,
		// but it costs nothing and the panel used to open with its heading already scrolled away
		// without it.
